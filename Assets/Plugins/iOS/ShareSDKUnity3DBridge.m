...
//  Unity-iPhone
//
//  Created by �?剑东 on 15/7/28.
//  Copyright (c) 2015�?mob. All rights reserved.
//
...
extern "C" {
     ...
     *  用户授权
     *
     *  @param reqID    流水�?     *  @param platType 平台类型
    ...
    /**
     *	@brief	一键分享内�?     *
     *  @param  reqID       流水�?     *	@param 	platTypes 	平台类型列表
     *	@param 	content 	分享内容
     *  @param  observer    观察回调对象名称
     */
     ...
     *  @brief  根据配置文件分享
     *
     *  @param reqID            流水�?     *  @param contentName      配置文件节点标识
...
extern "C" {
    ...
    void __setKakaoParams(NSDictionary *value,NSMutableDictionary *params,SSDKPlatformType subType)
    {
        ...
        if ([[value objectForKey:@"imageUrl"] isKindOfClass:[NSString class]])
        {
            ...
            {
                NSLog(@"#waring : 检测不到有效图片路�?请检查传入图片的路径的有效�");
            }
...
//            }
            ...
            if (customizeShareParams && [[customizeShareParams allKeys] count] > 0)
            {
                ...
                //人人�?                value = [MOBFJson objectFromJSONString:[customizeShareParams objectForKey:[NSString stringWithFormat:@"%lu",(unsigned long)SSDKPlatformTypeRenren]]];
                if ([value isKindOfClass:[NSDictionary class]])
                {
                    NSString *text = nil;
                    NSString *image = nil;
                ...
                //支付宝好�?                value = [MOBFJson objectFromJSONString:[customizeShareParams objectForKey:[NSString stringWithFormat:@"%lu",(unsigned long)SSDKPlatformTypeAliSocial]]];
                if ([value isKindOfClass:[NSDictionary class]])
                {
                    NSString *text = nil;
                    NSString *image = nil;
