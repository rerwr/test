    ...
    if([resp isKindOfClass:[PayResp class]]){
        ...
        switch (resp.errCode) {
            case WXSuccess:
                ...
                UnitySendMessage("Main Camera", "OnIOSWeChatPayReslut", [[NSString stringWithFormat:@"%d",resp.errCode] UTF8String]);
                break;
                
            default:
                strMsg = [NSString stringWithFormat:@"支付结果：取消"];
                UnitySendMessage("Main Camera", "OnIOSWeChatPayReslut", [[NSString stringWithFormat:@"%d",resp.errCode] UTF8String]);
                break;
        }
    ...
    if ([url.host isEqualToString:@"safepay"]) {
        ...
        [[AlipaySDK defaultService] processOrderWithPaymentResult:url standbyCallback:^(NSDictionary *resultDic) {
           ...
           NSString *code= [resultDic objectForKey:@"resultStatus"];
            NSLog(@"result = %@",[resultDic objectForKey:@"resultStatus"]);
            ...
            UnitySendMessage("Main Camera", "OnIOSWeChatPayReslut", [[resultDic objectForKey:@"resultStatus"] UTF8String]);
        }];
