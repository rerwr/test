...
public class UI_Control_ScrollFlow : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IPointerClickHandler
{
    ...
    public void RefreshNeedItems()
    {
        ...
        for (int i = 0; i < Current.NeedIds.Count; i++)
        {
            SetGrid(Current.NeedIds[i]);
        }
    }
    ...
    private void ClearNeedGrid()
    {
        for(int i=0;i<NeedGrid.childCount;i++)
        {
            if (NeedGrid.GetChild(i) != null)
            {

                GameObject.Destroy(NeedGrid.GetChild(i).gameObject);
            }
        }
    }
